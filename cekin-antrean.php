	...
	$RSKauth = $base64UrlHeader . "." . $base64UrlPayload . "." . $base64UrlSignature;
		...
		if (($header['x-token'] == $RSKauth ) && ($header['x-username']==$username)) {
			...
			if(empty($decode['kodebooking'])) {
				...
				$insertAPI= $con->query("insert into api_hit value('Cek In BPJS',NOW(),0,'201,Kode Boking $decode[kodebooking] , kode booking tidak boleh kosong')");
            }
				...
				$insertAPI= $con->query("insert into api_hit value('Cek In BPJS',NOW(),0,'201,Kode Boking $decode[kodebooking] , kode booking tidak ditemukan')");
			}
			else if ($statusbooking->num_rows > 0){
				...
				$insertAPI= $con->query("insert into api_hit value('Cek In BPJS',NOW(),0,'200,Kode Boking $decode[kodebooking]')");
			}
			else {
				$response = array(
						'metadata' => array(
					...
					$insertAPI= $con->query("insert into api_hit value('Cek In BPJS',NOW(),0,'201, Kode Boking $decode[kodebooking]')");
			}
		}
		if (($header['x-token'] != $RSKauth ) OR ($header['x-username'] !=$username)) {
			$response = array(
				...
				$insertAPI= $con->query("insert into api_hit value('Cek In BPJS',NOW(),0,'201,Token Tidak Valid $decode[kodebooking]')");
		}
		...
		$insertAPI= $con->query("insert into api_hit value('Cek In BPJS',NOW(),0,'201, Akses tidak diijinkan')");
