...
if ($method == 'POST') {
	...
	$RSKauth = $base64UrlHeader . "." . $base64UrlPayload . "." . $base64UrlSignature;
		...
		if (($header['x-token'] == $RSKauth ) && ($header['x-username']==$username)) {
				...
				$getkodeantrean="SELECT CASE WHEN MAX(nomorantrean)+1 IS NULL THEN 1 ELSE MAX(nomorantrean)+ 1 END as kodeantrean from que_antrean where tanggalperiksa='$tanggalperiksa' AND kode_antrean='B'";
				...
				if ($con->query($sqlins) === TRUE) {
					...
					$sqlcreate ="INSERT INTO que_antrean (kodebooking, nomorantrean, tanggalperiksa, waktu, estimasidilayani, jamdilayani,jenisDaftar,jenisPasien,klinik,app,waktu_tunggu,kode_antrean,status_panggil) VALUES ('$kodebookingrsk', '$kodeantreanrsk', '$tanggalperiksa', NOW(), '$estimate', '$jadwalmulai','RAJAL','BPJS','$rowID[DeptCode]','JKN','1','B','')";
