...
if ($method == 'POST') {
	$insertAPI= $con->query("insert into api_hit value('Get Antrean Farmasi',NOW(),0,'-')");
	...
	$RSKauth = $base64UrlHeader . "." . $base64UrlPayload . "." . $base64UrlSignature;
		...
		if (($header['x-token'] == $RSKauth ) && ($header['x-username']==$username)) {
			...
			$jumlah_karakter = strlen($decode['kodebooking']);
			...
			if (!$decode['kodebooking']){
				...
				$getJadwalOperasi = $con->query("SELECT antrean_farmasi FROM quebpjs where kodebooking ='$decode[kodebooking]'");
				if ($getJadwalOperasi->num_rows > 0) {
					...
					$response = array(
						'response'=>array(
							...
							'nomorantrean' => $data['antrean_farmasi'],
							'keterangan' => "-"
							),
						'metadata' => array(
							'message' => 'Ok',
				...
				$insertAPI= $con->query("insert into api_hit value('Ambil antrean farmasi',NOW(),0)");
			}	
			else {
				$response = array(
						'response' => array(
		...
		$insertAPI= $con->query("insert into api_hit value('Get Antrean Farmasi',NOW(),0,'akses tidak diijinkan')");
		echo json_encode($response);
