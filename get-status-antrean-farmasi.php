...
if ($method == 'POST') {
	$insertAPI= $con->query("insert into api_hit value('Get Status Antrean Farmasi',NOW(),0,'-')");
	...
	$RSKauth = $base64UrlHeader . "." . $base64UrlPayload . "." . $base64UrlSignature;
				...
				if ($getJadwalOperasi->num_rows > 0) {
					...
					$response = array(
						'response'=>array(
							...
							'sisaantrean' => 0,
							'antreanpanggil' => 0,
							'keterangan' => "Maaf,Informasi antrean belum dapat ditampilkan , Terima Kasih"
							),
						'metadata' => array(
							'message' => 'Ok',
							'code' => 200
		...
		$insertAPI= $con->query("insert into api_hit value('Get Status Antrean Farmasi',NOW(),0,'akses tidak diijinkan')");
		echo json_encode($response);
