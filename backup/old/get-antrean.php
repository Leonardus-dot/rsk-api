...
if ($method == 'POST') {
	...
	$RSKauth = $base64UrlHeader . "." . $base64UrlPayload . "." . $base64UrlSignature;
		...
		if (($header['x-token'] == $RSKauth )) {
				...
				$getkodebooking="SELECT MAX(kodebooking) as kodebooking from que_antrean";
				...
				if ($con->query($sqlins) === TRUE) {
					...
					$sqlcreate ="INSERT INTO que_antrean (kodebooking, nomorantrean, tanggalperiksa, waktu, estimasidilayani, jamdilayani,jenisDaftar,jenisPasien,klinik) VALUES ('$kodebookingrsk', '$kodeantreanrsk', '$tanggalperiksa', NOW(), '$estimate', '$jadwalmulai','RAJAL','BPJS','$rowID[DeptCode]')";
