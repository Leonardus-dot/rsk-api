...
if ($method == 'POST') {
	...
	$insertAPI= $con->query("insert into api_hit value('Get Operasi',NOW(),0,'')");
	...
	$RSKauth = $base64UrlHeader . "." . $base64UrlPayload . "." . $base64UrlSignature;
		...
		if (($header['x-token'] == $RSKauth ) && ($header['x-username']==$username)) {
			...
			if (!$decode['nopeserta']){
				...
				$getJadwalOperasi = $con->query("SELECT * FROM operasi INNER JOIN department ON operasi.kodepoli = department.DeptCode WHERE nopeserta='$decode[nopeserta]' AND tipepasien='BPJS' and tanggaloperasi >= date(NOW())");
				...
				$insertAPI= $con->query("insert into api_hit value('Ambil Jadwal Operasi',NOW(),0)");
			}	
			else {
				$response = array(
						'response' => array(
