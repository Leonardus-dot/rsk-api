	...
	$RSKauth = $base64UrlHeader . "." . $base64UrlPayload . "." . $base64UrlSignature;
		...
		if (($header['x-token'] == $RSKauth ) && ($header['x-username']==$username)) {
			...
			if (empty($decode['tanggalawal'])){
				...
				$getJadwalOperasi = $con->query("SELECT * FROM operasi INNER JOIN department ON operasi.kodepoli = department.DeptCode WHERE tanggaloperasi >='$decode[tanggalawal]' and tanggaloperasi <='$decode[tanggalakhir]' AND tipepasien='BPJS' and terlaksana in ('0','1','MENUNGGU','TERKONFIRMASI DATANG')");
				...
				if ($getJadwalOperasi->num_rows > 0) {
					...
					while($data = $getJadwalOperasi->fetch_assoc()) {
						...
						else if($data['terlaksana']=='TERKONFIRMASI DATANG'){
							$terlaksana="0";
						}
						else{
				...
				$insertAPI= $con->query("insert into api_hit value('Get Operasi',NOW(),0,'Tanggal $decode[tanggalawal] - $decode[tanggalakhir]')");
